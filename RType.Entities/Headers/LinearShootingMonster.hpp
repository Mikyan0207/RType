...
class LinearShootingMonster final : public IMonster
{
public:
	LinearShootingMonster();
	LinearShootingMonster(const LinearShootingMonster&);
	LinearShootingMonster(LinearShootingMonster&&) noexcept;
	~LinearShootingMonster() override = default;

public:
	LinearShootingMonster& operator=(const LinearShootingMonster&) = delete;
	LinearShootingMonster& operator=(LinearShootingMonster&&) noexcept = delete;
...
private:
	...
	static constexpr float SHOOT_FREQUENCY = 0.2;
};

extern "C" RTYPEENTITIES_API IMonster * Instantiate(void)
{
	return new LinearShootingMonster;
}
